...
class productController extends mainModel
{
	...
	public function registrarProductoControlador()
	{
		...
		$precio_compra = $this->limpiarCadena($_POST['producto_costo_compra']);
		$costo_envio = $this->limpiarCadena($_POST['producto_costo_envio']);
		$precio_venta = $this->limpiarCadena($_POST['producto_precio_vender']);
		$costo_unidad = $this->limpiarCadena($_POST['producto_costo_unidad']);
		...
		if ($codigo == "" || $nombre == "" || $precio_compra == "" || $precio_venta == "" || $stock == "" || $costo_envio == "" || $costo_unidad == "" || $fecha_ingresado == "") {
			$alerta = [
				"tipo" => "simple",
		...
		if ($this->verificarDatos("[0-9.]{1,25}", $costo_envio)) {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "El COSTO DEL ENVIO no coincide con el formato solicitado",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
		}
		if ($this->verificarDatos("[0-9.]{1,25}", $costo_unidad)) {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "El COSTO POR UNIDAD no coincide con el formato solicitado",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
		...
		# Comprobando precio INDIVIDUAL del producto #
		$costo_unidad = number_format($costo_unidad, MONEDA_DECIMALES, '.', '');
		if ($costo_unidad <= 0) {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "El COSTO POR UNIDAD no puede ser menor o igual a 0",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
		...
		if ($costo_unidad > $precio_venta) {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "El precio por unidad del producto no puede ser mayor al precio de venta",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
		...
		$producto_datos_reg = [
			...
			[
				...
				"campo_marcador" => ":CostoEnvio",
				"campo_valor" => $costo_envio
			],
			[
				"campo_nombre" => "producto_costo_unidad",
				"campo_marcador" => ":CostoUnidad",
				"campo_valor" => $costo_unidad
			],

			[
				"campo_nombre" => "producto_precio_vender",
				"campo_marcador" => ":PrecioVender",
				"campo_valor" => $precio_venta
			],
			[
				"campo_nombre" => "producto_fecha_ingresado",
				"campo_marcador" => ":FechaIngresado",
				"campo_valor" => $fecha_ingresado
			],
			[
	...
	/*----------  Controlador listar producto  
		public function listarProductoControlador($pagina,$registros,$url,$busqueda,$categoria){
			...
			$campos="producto.producto_id,producto.producto_codigo,producto.producto_nombre,producto_stock_total,producto.producto_precio_vender,producto.producto_foto,categoria.categoria_nombre";
		                ...
		                <div class="media-content">
		                    <div class="content">
		                        <p>
		                            ...
		                            <strong>PRECIO:</strong> $'.$rows['producto_precio_vender'].', 
	...
	public function listarProductoControlador($pagina, $registros, $url, $busqueda, $categoria)
	{
		...
		if ($total >= 1 && $pagina <= $numeroPaginas) {
            ...
            <div class="column is-3">
                <div class="card card-producto">
                    ...
                    <div class="card-image">
                        <figure class="image img-producto">';

				if (is_file("./app/views/productos/" . $rows['producto_foto'])) {
					$tabla .= '<img src="' . APP_URL . 'app/views/productos/' . $rows['producto_foto'] . '" class="img-fit">';
				} else {
					$tabla .= '<img src="' . APP_URL . 'app/views/productos/default.png" class="img-fit">';
				}
				...
				$tabla .= '
                    ...
                    <div class="card-content">
                        <p class="title is-6">' . $contador . ' - ' . $rows['producto_nombre'] . '</p>
                        <p class="subtitle is-7">
                            <strong>Código:</strong> ' . $rows['producto_codigo'] . '<br>
                            <strong>Precio:</strong> $' . $rows['producto_precio_vender'] . '<br>
                            <strong>Stock:</strong> ' . $rows['producto_stock_total'] . '<br>
                    ...
                    <footer class="card-footer">
                        ...
                        <a href="' . APP_URL . 'productPhoto/' . $rows['producto_id'] . '/" class="card-footer-item icono-accion">
			...
			$tabla .= '<p class="has-text-centered">No hay productos registrados</p>';
		}

		if ($total > 0 && $pagina <= $numeroPaginas) {
			$tabla .= '<p class="has-text-right">Mostrando <strong>' . $pag_inicio . '</strong> a <strong>' . $pag_final . '</strong> de <strong>' . $total . '</strong></p>';
			$tabla .= $this->paginadorTablas($pagina, $numeroPaginas, $url, 7);
		}
	...
	public function actualizarProductoControlador()
	{
		...
		if ($this->verificarDatos("[0-9.]{1,25}", $costo_envio)) {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "El COSTO DE ENVÍO no coincide con el formato solicitado",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
