...
class saleController extends mainModel
{
	...
	public function agregarProductoCarritoControlador()
	{
		...
		if (empty($_SESSION['datos_producto_venta'][$codigo])) {
			...
			$detalle_total = $detalle_cantidad * $campos['producto_precio_vender'];
			...
			$_SESSION['datos_producto_venta'][$codigo] = [
				...
				"venta_detalle_precio_compra" => $campos['producto_costo_unidad'],
				"venta_detalle_precio_venta" => $campos['producto_precio_vender'],
				"venta_detalle_cantidad" => 1,
				"venta_envio_costo" => $campos['producto_envio_venta'],
			...
			$detalle_envio = ($_SESSION['datos_producto_venta'][$codigo]['venta_envio_costo']); // Obtenemos el costo de envío actualizado desde el formulario
			$stock_total = $campos['producto_stock_total'] - $detalle_cantidad;
			...
			$_SESSION['datos_producto_venta'][$codigo] = [
				...
				"venta_envio_costo" => $detalle_envio,
				"venta_detalle_total" => $detalle_total,
				"venta_detalle_descripcion" => $campos['producto_nombre']
			];
	...
	{
		/*== Recuperando código, cantidad y costo de envío del producto ==*/
		$codigo = $this->limpiarCadena($_POST['producto_codigo']);
		...
		if ($codigo == "" || $cantidad == "" || $envio == "") {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "No podemos actualizar el producto debido a que faltan algunos parámetros de configuración",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
		...
		/*== Comprobando valores válidos ==*/
		if ($cantidad <= 0 || $envio < 0) {
			$alerta = [
				"tipo" => "simple",
				"titulo" => "Ocurrió un error inesperado",
				"texto" => "Debes de introducir una cantidad mayor a 0 y un costo de envío válido",
				"icono" => "error"
			];
			return json_encode($alerta);
			exit();
		...
		if (!empty($_SESSION['datos_producto_venta'][$codigo])) {
			if (
				$_SESSION['datos_producto_venta'][$codigo]["venta_detalle_cantidad"] == $cantidad &&
				$_SESSION['datos_producto_venta'][$codigo]["venta_envio_costo"] == $envio
			) {
				$alerta = [
					"tipo" => "simple",
					"titulo" => "Ocurrió un error inesperado",
					"texto" => "No has modificado ni la cantidad ni el costo de envío",
					"icono" => "error"
				];
				return json_encode($alerta);
				exit();
			...
			$_SESSION['datos_producto_venta'][$codigo] = [
				...
				"venta_detalle_precio_venta" => $campos['producto_precio_vender'],
				"venta_detalle_cantidad" => $detalle_cantidad,
				"venta_envio_costo" => $envio, // Actualizamos el costo de envío
				"venta_detalle_total" => $detalle_total,
				"venta_detalle_descripcion" => $campos['producto_nombre']
			];

			$_SESSION['alerta_producto_agregado'] = "Se $diferencia_productos <strong>" . $campos['producto_nombre'] . "</strong> a la venta. Total en carrito: <strong>$detalle_cantidad</strong>. Costo de envío actualizado: <strong>" . number_format($envio, MONEDA_DECIMALES, '.', ',') . "</strong>";
	...
	public function registrarVentaControlador()
	{
		...
		/*== Calcular el total de envío ==*/
		$venta_envio = 0;
		foreach ($_SESSION['datos_producto_venta'] as $producto) {
			$venta_envio += floatval($producto['venta_envio_costo']);
		}
		$venta_envio = number_format($venta_envio, MONEDA_DECIMALES, '.', '');
		...
		$datos_venta_reg = [
			...
			[
				"campo_nombre" => "venta_envio",
				"campo_marcador" => ":Envio",
				"campo_valor" => $venta_envio
			],
			[
	...
	public function listarVentaControlador($pagina, $registros, $url, $busqueda)
	{
		...
		$campos_tablas = "venta.venta_id,venta.venta_codigo,venta.venta_fecha,venta.venta_hora,
			venta.venta_total,venta.usuario_id,venta.cliente_id,venta.caja_id,
			usuario.usuario_id,usuario.usuario_nombre,usuario.usuario_apellido,
			cliente.cliente_id,cliente.cliente_nombre,cliente.cliente_apellido,
			COALESCE(SUM(vd.venta_detalle_precio_compra),0) AS descuento";
			...
			foreach ($datos as $rows) {
				$tabla .= '
						<tr class="has-text-centered" >
							...
							<td>' . MONEDA_SIMBOLO . number_format($rows['venta_total'] - $rows['descuento'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR) . ' ' . MONEDA_NOMBRE . '</td>
			                <td>
